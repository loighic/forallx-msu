...
Let's start with these sentences:
	\begin{earg}
		\item[\ex{and1}] Jeff is athletic, and Mary is also athletic.
		\item[\ex{and2}] Mary and Jeff are both athletic.
		\item[\ex{and3}] Jeff is not athletic, but Mary is.
	\end{earg}
We will need separate sentence letters to symbolize sentences \ref{and1} and \ref{and2}, and so we will use this symbolization key:
	\begin{ekey}
		\item[J] Jeff is athletic.
		\item[M] Mary is athletic.
	\end{ekey}
Sentence \ref{and1} can be partially symbolized as `$J$ and $M$'. To symbolize the `and'. We will use `\eand', which is called the \textit{ampersand}. Thus, sentence \ref{and1} becomes `$(J\eand M)$'. When two sentences are connected with an `$\eand$', the resulting sentence is called a \define{conjunction}. The two sentences that are combined with the `$\eand$' are the \define{conjuncts} of the conjunction. So, `$J$' and `$M$' are the conjuncts of the conjunction `$(J\eand M)$'.

Although it is worded differently, sentence \ref{and2} has the same meaning as sentence \ref{and1}. Thus, it is also symbolized as `$(J\eand M)$'.
...
For sentence \ref{and3}, let's first symbolize `Jeff is not athletic' as `$\enot J$'. `Mary is' means `Mary is athletic', and so that subsentence is symbolized as `$M$'. `But' may have a slightly different meaning in English than `and', but, grammatically, they serve the same role: to join two sentences as a conjunction. Putting this altogether, sentence \ref{and3} is symbolized as `$(\enot J \eand M)$'.
...
We will start with these sentences:
	\begin{earg}
		\item[\ex{or1}] Either Jeff is at the train station, or Kate driving to Santa Fe.
		\item[\ex{or2}] Jeff or Sarah is at the train station. 
	\end{earg}
And we will use this symbolization key:
	\begin{ekey}
		\item[J] Jeff is at the train station.
		\item[K] Kate is driving to Santa Fe.
		\item[S] Sarah is at the train station.
	\end{ekey}
To represent the `or' in sentences \ref{or1} and \ref{or2}, we will use the symbol `$\eor$' (which we call the \textit{wedge}, not \textit{v}). Sentence \ref{or1}, then, is written as `$(J \eor K)$'. When two sentences are connected with an `$\eor$', the resulting sentence is called a \define{disjunction}. `$J$' and `$K$' are the \define{disjuncts} of the disjunction `$(J \eor K)$'.

Sentence \ref{or2} is only slightly more complicated. We can paraphrase it as `Either Jeff is at the train station, or Sarah is at the train station', and then we symbolize it as `$(J \eor S)$'.
...
Sentences \ref{or3} and \ref{or4} are different, and they have different meanings. Consequently, they need to be translated into TFL differently. According to sentence \ref{or4}, neither one is at the train station. Therefore, we can paraphrase it like this:
\begin{ebullet}
...
That's how we translate an English sentence containing \textit{if and only if} into TFL. But let's think about what \textit{if and only if} means. Here's another set of sentences.
	\begin{earg}
...
We call `$(D \eiff S)$' a \define{biconditional}, because it is equivalent to the two conditionals that have their antecedent and the consequent switched (as in sentence \ref{iff4}).
