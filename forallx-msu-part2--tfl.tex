...
When two sentences are connected with an `$\eand$', the resulting sentence is called a \define{conjunction}. The two sentences that are combined with the `$\eand$' are the \define{conjuncts}. So, $S$ and $K$ are the conjuncts of the conjunction $(S \eand K)$.
