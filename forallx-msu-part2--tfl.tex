...
and this one:
	...
	\end{earg}
Both arguments are valid, and, as perhaps you  can see, they share a common form. If we want to represent this form only, we can do so this way:
	\begin{earg}
		\item[1.] If A, then C
		\item[2.] A
		\item[3.] Therefore, C
	\end{earg}
Any argument with this form will be valid. It doesn't matter what English sentences are put in the places of A, B, and C. 

Now, let's look at this argument:
	\begin{earg}
		\item[1.] Mary is either happy or sad.
		\item[2.] Mary is not happy.
		\item[3.] Therefore, Mary is sad.
	\end{earg}
...
Here is another example:
	\begin{earg}
		\item[1.] It's not the case that Jeff both studied often and acted in lots of plays.
		\item[2.] Jeff acted in lots of plays.
		\item[3.] Therefore, Jeff did not study often.
	\end{earg}
...
These examples illustrate what we can call \emph{validity in virtue of form}. The four arguments about Mary and Jeff are valid, but their being valid has nothing to do with the specific meaning of `Mary is sad', `Leiser is a spy', or `Jeff acted in lots of plays'. Instead, these arguments are valid in virtue of the meanings of just these words: \textit{and}, \textit{or}, \textit{not}, and \textit{if\ldots, then\ldots}. 
...
There are plenty of arguments that are valid, but not because of the argument's form. This an example:
	...
	\end{earg}
It is impossible for the premise to be true and the conclusion to be false, and so the argument is valid. But, in this case, an argument with the same form might not be valid, as we find here:
	\begin{earg}
		\item[1.] Juanita is a vixen.
...
Again, it is impossible for the premise to be true and the conclusion false, for nothing can be both green all over and red all over. So, the argument is valid. Here, however, is an argument with the same form that is not valid:
	...
	\end{earg}
Obviously, a sculpture (or anything) can be green all over and shiny all over, and so this argument is not valid. 

Much more can be said about arguments that are valid but not because of their form. That will not be our focus here, though. From this point forward, we will be interested only in arguments that are valid or invalid because of their form.
...
Consider this sentence again:
	...
	\end{earg}
We call `It is raining outside' and `Mary is miserable' \textit{subsentences} of (a). To specify the structure of the arguments in \S\ref{s:ValidityInVirtueOfForm}, we replaced the sentences and subsentences in each argument with individual letters. For instance, `It is raining outside' with `A' in the first argument. This kind of representation---letters standing for sentences or subsentences---is central to the formal language that we develop in this book.

\begin{factboxy}{Atomic sentences}
An \define{atomic sentence} is a sentence that (1) can be true or false and (2) cannot be broken down into smaller sentences.\smallskip

An atomic sentence of English is a declarative sentence that does not contain any subsentences.\smallskip

An atomic sentence in our logic system is a single capital letter.
\end{factboxy}

Atomic sentences are the basic building blocks used to form more complex sentences. We will use uppercase Roman letters for atomic sentences in our logic system. There are only twenty-six letters of the alphabet, but there is no limit to the number of atomic sentences that we might want to designate. By adding subscripts to letters, we obtain new atomic sentences. Here, for instance, are five different atomic sentences:
	$$A, R, R_1, R_2, A_{234}$$
We will use atomic sentences to represent, or \emph{symbolize}, certain English sentences. To do this, we provide a \define{symbolization key}, such as the following.
	...
	\end{ekey}
When we do this, we are not fixing this symbolization once and for all. We are just saying that, for the time being, we will think of the atomic sentence `$A$' as symbolizing the English sentence `It is raining outside', and the atomic sentence of TFL, `$C$', as symbolizing the English sentence `Mary is miserable'. Later, when we are dealing with different sentences or different arguments, we can provide a new symbolization key; for instance, 
	\begin{ekey}
		\item[A] Jeff studied often.
		\item[C] Jeff acted in lots of plays.
	\end{ekey}
...
The second sentence is partially symbolized as `It is not the case that $B$'. In order to complete the symbolization, we need a symbol for `it is not the case that'. Or, put differently, a symbol that, when added to $B$ will express `the negation of $B$'. We will use `\enot' and symbolize sentence \ref{not2} as `$\enot B$'.
...
Setting aside the `$\enot$' for a moment, each of the other logical operators, as you know, combine two sentences (which may, themselves, be composed of multiple sentences). The two sentences that are being combined by a logical operator comprise (with the operator itself) that operator's \define{scope}. 
...
When the scope of a logical operator is the entire sentence, then that logical operator is the \define{main logical operator}. So, in the previous example, the `$\eor$' is the main logical operator, and consequently, the sentence is a disjunction. (The subsentences are different kinds of sentences, but the whole sentence is a disjunction.)
