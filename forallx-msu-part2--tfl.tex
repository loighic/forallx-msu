...
When we are dealing with a \emph{sentence} other than an atomic sentence, we can see that there must be some sentential connective that was introduced \emph{last}, when constructing the sentence. We call that connective the \define{main logical operator} of the sentence. In the simplest case, when there is only one connective---for instance, as there is here: $(P \eand Q)$---the main logical operator is the one connective, the `$\eand$'. If we add to this sentence, for instance like this: $((P \eand Q) \eor R)$, then the main logical operator becomes `$\eor$'. This is not only the operator that was added last, but it also dictates, at the most general level, what kind of sentence this is: it's a disjunction. (And one of the disjuncts happens to be a conjunction.) 

Adding to this sentence again, we get $(((P \eand Q) \eor R) \eif T)$. Now the main logical operator is `$\eif$', and the sentence is a conditional. And finally, if we negate the sentence: $\enot(((P \eand Q) \eor R) \eif T)$, `$\enot$' becomes the main logical operator. Some more  examples:
\begin{earg}
...
\item[\ex{logic-operator4}] $(P \eand \enot (\enot Q \eor R))$ ~~~The main logical operator is `$\eand$'. 
\item[\ex{logic-operator5}] $((\enot E \eor F) \eif \enot G)$ ~~~The main logical operator is `$\eif$'.
\end{earg}

When the sentence includes the outermost brackets, you can find the main logical operator for a sentence by using the following method:
\begin{ebullet}
...
As we will discuss in the next section (\S\ref{TFLconventions}), it is, in some cases, acceptable to omit the outermost brackets. For instance, although it is not strictly allowable according to the rules given in \S\ref{s:Sentences}, we can write `$(P \eand R) \eif Q$' instead of `$((P \eand R) \eif Q)$'. When the outermost brackets are omitted, `$\enot$' won't be the main logical operator, and so (1) will not apply. In other words, if `$\enot$' is the main logical operator, then the outermost brackets have to be used. But remember, when `$\enot$' is the main logical operator---as it is in this example: $\enot(((P \eand Q) \eor R) \eif T)$---the `$\enot$' will be outside the outermost brackets. When the outermost brackes are omitted, (2a) and (2b) can be used, but stop when your count gets to zero instead of $1$.
...
Finally, let's give a formal definition of the \emph{scope} of a negation (which was mentioned in \S\ref{s:ConnectiveConjunction}). The scope of a `$\enot$' is the subsentence for which `$\enot$' is the main logical operator. Consider a sentence like:
$$(P \eand (\enot (R \eand B) \eiff Q))$$
which was constructed by conjoining `$P$' with `$ (\enot (R \eand B) \eiff Q)$'. This last sentence was constructed by placing a biconditional between `$\enot (R \eand B)$' and `$Q$'. The former of these sentences---a subsentence of our original sentence---is a sentence for which `$\enot$' is the main logical operator. So the scope of the negation is just `$\enot(R \eand B)$'. The same holds for every connective, and so we have the following definition.
