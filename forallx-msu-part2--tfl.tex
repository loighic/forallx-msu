...
As we will discuss in the next section (\S\ref{TFLconventions}), it is, in some cases, acceptable to omit the outermost brackets of a sentence. For instance, although it is not strictly allowable give the rules in \S\ref{s:Sentences}, we can write `$((P \eand R) \eif Q)$' as `$(P \eand R) \eif Q$'. 

When the outermost brackets are omitted, `$\enot$' will not be the main logical operator, and so (1) will not apply. In other words, if `$\enot$' is the main logical operator, then the outermost brackets have to be used. But remember, just as in $\enot(((P \eand Q) \eor R) \eif T)$, when `$\enot$' is the main logical operator, it will be outside the outermost brackets. (2a) and (2b) can be used, but stop when your count gets to zero instead of $1$.
...
The recursive structure of sentences in TFL also allows us to give a formal definition of the \emph{scope} of a negation (mentioned in \S\ref{s:ConnectiveConjunction}). The scope of a `$\enot$' is the subsentence for which `$\enot$' is the main logical operator. Consider a sentence like:
