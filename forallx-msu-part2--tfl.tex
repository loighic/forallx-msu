...
And for these sentences, we will use this symbolization key:
	...
	\end{ekey}
To represent the `or' in sentences \ref{or1} and \ref{or2}, we will use the symbol `$\eor$' (which we call the \textit{wedge}, not \textit{v}). Sentence \ref{or1}, then, is written as `$(F \eor M)$'. When two sentences are connected with an `$\eor$', the resulting sentence is called a \define{disjunction}. `$F$' and `$M$' are the \define{disjuncts} of the disjunction `$(F \eor M)$'.
...
To use 1 -- 7, you have to drill down to each part of the sentence. For instance, (4) tells us that if \meta{A} and \meta{B} are sentences, then $(\meta{A}\eor\meta{B})$ is a sentence. Thus, to know if $(\meta{A}\eor\meta{B})$ is a sentence, we have to determine if \meta{A} and \meta{B} themselves are sentences of TFL. Let's look at an example:

\begin{earg}
\item[] $(P \eiff T) \eor Q$
\end{earg}

\noindent According to (4), this is a sentence if `$(P \eiff T)$' is a sentence and if `$Q$' is a sentence. Are they? According to (1), every atomic sentence---that is, every individual capital letter in this font: $A, B, C, \ldots$---is a sentence of TFL, and so `$Q$' is a sentence. Is `$(P \eiff T)$' a sentence of TFL? According to (6), it is as long as `$P$' and `$T$' are both sentences---which they are since both are atomic sentences. Thus, `$(P \eiff T) \eor Q$' is a sentence of TFL. 
...
Even though the sentence is a conditional, the negation and the conjunction each have a role. But their roles are limited to only a part of the sentence. Take the negation. It applies to (i.e., negates) \textit{today is Saturday}.  
The conjunction's job, meanwhile, is to join \textit{Amy is at work} and \textit{Kate is at school}. 
...
Finally, let's define the \define{scope} of a logical operator. Basically, the scope of a logical operator is the part of the sentence to which it applies. We give the precise definition in terms of the main logical operator for the whole sentence and the main logical operators for any sub-sentences contained therein. 

\begin{factboxy}{Scope}
The \define{scope} of a logical operator (in a sentence) is the sentence or sub-sentence for which that logical operator is the main logical operator.
\end{factboxy}
