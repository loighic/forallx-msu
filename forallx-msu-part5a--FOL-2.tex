...
\begin{ebullet}
\item[]$\forall x[F(x) \eif G(x)]$: For all $x$, if $x$ is an $F$, then $x$ is a $G$’ \textbf{or}%\smallskip 
\item[]\hspace{29mm}`Every $F$ is $G$.’\smallskip
\item[]$\exists x[F(x) \eand G(x)]$:`There exists an $x$, such that $x$ is an $F$ and a $G$’ \textbf{or}%\smallskip
