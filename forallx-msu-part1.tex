...
We can also say that, in an argument, the conclusion follows from the premises.\\
\end{factboxy}

That's the definition, but a broader analysis of arguments must include that they can be good or bad---or  somewhere inbetween. A good argument is one in which the premises do, in fact, support the conclusion. For such an argument, if the premises are true, then we have good reason to believe that the conclusion is true. On the other hand, a bad (or a weak) argument is still an argument. It is just one in which the premises provide little support for the conclusion.
...
In the definition of an argument, we said that each premise and the conclusion is a sentence. And, as we saw, both premises and the conclusion in the example are individual sentences. All arguments can be expressed this way and many are, but a single sentence can also contain a complete argument, as is shown here:
	...
	\end{quote}
This argument has one premise and a conclusion. The premise and the conclusion could both be individual sentences, but here they are just clauses separated by the `and'. (The premise is before the `and', and the conclusion is after it.)
...
When approaching an argument, we want to know whether or not the conclusion is supported by the premises. So, the first thing to do is to identify the premise or premises (the sentences providing support) and the conclusion (and the sentence being supported). As a guide, these words are often used to indicate that a sentence or clause is the conclusion of an argument:
	\begin{quote}
...
To be a premise or conclusion of an argument, it must possible for the sentence to be true or false. So, for our purposes, by \define{sentence}, we mean a declarative sentence. In contrast to declarative sentences, the following are types of sentences that cannot be true or false, and so they cannot be part of an argument.

\paragraph{Questions} `Are you sleepy yet?' is, obviously, a question. Although you might be sleepy or you might be alert, the question itself is neither true nor false. For this reason, questions will not count as sentences in logic. Suppose you answer the question: `I am not sleepy.' This is either true or false, and so it is a sentence in the logical sense. Generally, \emph{questions} will not count as sentences, but \emph{answers} will. 

\paragraph{Imperatives} Imperative sentences are, essentially, commands (although they can be nicer than what we usually think of as a command). For instance, `Wake up!', `Sit up straight', and `Tell me how to set the table' are imperatives. Although it might be good for you to sit up straight or it might not, and you may or may not do it, the command is neither true nor false. Note, however, that commands are not always phrased as imperatives.  As Cartman might say,``You will respect my authority.'' This is a command, but it is also true or false--- either you will or you will not--- and so it counts as a sentence in the logical sense.

\paragraph{Exclamations} Some exclamatory sentences can be true or false (and so they are also declarative sentences) and some cannot be. `It's Friday!' is an exclamation, and it is true or false. So, it can be part of an argument. On the other hand, a sentence such as `Ouch!' is neither true nor false, and so it cannot. 

\section{Truth values}
The kinds of sentences that cannot be part of an argument are not \emph{assertoric}: they cannot be true or false. For instance, it does not even make sense to ask whether a question is true. It only makes sense to ask whether it is true that someone asked the question or whether the answer is true.

 The premises and conclusion of an argument, meanwhile, must be capable of having a \define{truth value}. That is, we must be able to assign a value about its truth to each sentence in an argument. The two truth values that concern us are `true' and `false'. 
...
Consider this argument:
	...
	\end{earg}
If the premises of this argument are true---which, as it turns out, they are---it is very likely that the conclusion is true. But it is possible that someone besides a logic student is reading this book. If, say, the roommate of the book's owner picked it up and began looking through it, he or she would not immediately become a logic student. So, for this argument, it is likely that the conclusion is true if the premises are, but it's not guaranteed. 

Now, let's look at this example:
	\begin{earg}\label{valid-oranges}
		\item[1.] Oranges are either fruits, or they are mammals.
		\item[2.] Oranges are not mammals.
		\item[3.] Therefore, oranges are fruits.
	\end{earg}
...
Here is another example,
	\begin{earg}
		\item[1.] Oranges are either insects, or they are fish.
		\item[2.] Oranges are not insects.
		\item[3.] Therefore, oranges are fish.
	\end{earg}
Although this argument might strike you as a little bit odd, we can say almost the exact same thing about this one as we did for the previous one:
\begin{quote}
...
This brings us to an important definition as well as an important point about our task as logicians. First the definition. 
\begin{factboxy}{Valid}
An argument is \define{valid} (or \define{deductively valid}) when, and only when, it is impossible for all of the premises to be true and the conclusion to be false.\\
Every argument that does not satisfy this definition is \define{invalid} (or \define{deductively invalid}).
\end{factboxy}
\noindent As logicians, we want to determine when the conclusion of an argument follows from the premises with certainty. From the perspective of logic, whether the premises actually are true is less important to us. Of course, determining whether or not they are can be important, but that is normally left to historians, scientists, or the Hardy boys.

So, we want to know whether, if all the premises \textit{were} true, would the conclusion also have to be true? Consider this argument:
	\begin{earg}
		\item[1.] Oranges are either fruits or aliens from another galaxy.
		\item[2.] Oranges are not fruits.
		\item[3.] Therefore, oranges are aliens from another galaxy.
	\end{earg}
This argument is valid. \emph{If} both premises are true, \emph{then} the conclusion has to be true. Now, let's think about this argument:
	\begin{earg}
		\item[1.] London is in England.
		\item[2.] Beijing is in China.
		\item[3.] Therefore, Paris is in France.
	\end{earg}
The premises and conclusion of this argument are all true, but the argument is invalid. If Paris were to declare independence from the rest of France, then the conclusion would be false, even though both of the premises would remain true. Thus, it is \emph{possible} for the premises of this argument to be true and the conclusion false. Hence, the argument is invalid.

The important thing to remember is that validity is not about the actual truth or falsity of the sentences in the argument. It is about whether it is \emph{possible} for all the premises to be true and the conclusion false. (Or, to say the same thing in a different way, whether, the conclusion has to be true \textit{if} all of the premises are true.)

We can, however, classify the arguments that are valid and have all true premises. These we call \define{sound}\label{def-sound-arg}. The second argument on p.~\pageref{valid-oranges} is sound.
...
Many good arguments are invalid. Consider this one:
	\begin{earg}
		...
		\item[5.] In January 2020, it rained in London.
		\item[6.] In January 2021, it rained in London.
		\item[7.] In January 2022, it rained in London.
	\item[8.] Therefore, every January, it rains in London.
\end{earg}
This argument generalizes from observations about several cases to a conclusion about all cases. This is one type of \define{inductive} argument. This argument could be made stronger by adding additional premises, for instance: `In January 2015, it rained in London,' `In January 2014, it rained in London,' and so on. But, however many premises like this we add, the argument will remain invalid. Even if it has rained in London every January for the past 10,000 years, it remains \emph{possible} that it won't rain in London next January.

Inductive arguments---even good inductive arguments---are not valid. Unlikely though it might be, it is \emph{possible} for their conclusions to be false, even when all of their premises are true. In this book, we will set aside the question of what makes strong versus weak inductive arguments. Our interest is in understanding valid arguments and distinguishing them the invalid ones.  
...
\begin{factboxy}{jointly possible and jointly impossible}
Sentences are \define{jointly possible} when, and only when, it is possible for them all to be true together.\\
Sentences are \define{jointly impossible} when, and only when, it is \textit{not} possible for them all to be true together.
\end{factboxy}
...
Consider these sentences:
	...
	\end{earg}
In order to know if sentence \textit{a} is true, you would need to look outside or check the weather channel. It might be true; it might be false. A sentence which is capable of being true and capable of being false (in different circumstances, of course) is \define{contingent}.
