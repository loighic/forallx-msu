...
That's the definition of an argument, but a broader analysis must include that arguments can be good or bad---or  somewhere inbetween. A good argument is one in which the premises do, in fact, support the conclusion. For such an argument, if the premises are true, then we have good reason to believe that the conclusion is true. On the other hand, a bad (or a weak) argument is still an argument. It is just one in which the premises provide little support for the conclusion.
...
\paragraph{Imperatives} Imperative sentences are, essentially, commands (although they can be nicer than what we usually think of as a command). For instance, `Wake up!', `Sit up straight', and `Please, tell me how to set the table' are imperatives. Although it might be good for you to sit up straight or it might not, and you may or may not do it, the command is neither true nor false. Note, however, that commands are not always phrased as imperatives.  As Cartman might say,``You will respect my authority.'' This is a command, but it is also true or false--- either you will or you will not--- and so it counts as a sentence in the logical sense.

\paragraph{Exclamations} Some exclamatory sentences can be true or false (and so they are also declarative sentences) and some cannot be. `It's Friday!' is an exclamation, and it is true or false. So, it can be part of an argument. On the other hand, a sentence such as `Ouch!' is neither true nor false, and so it cannot be part of an argument. 
...
So, we want to know whether, if all the premises \textit{were} true, would the conclusion also have to be true? Consider this argument:
	\begin{earg}
		\item[1.] Oranges are either fruits, or they are aliens from another galaxy.
...
The important point to remember is that validity is not about the actual truth or falsity of the sentences in the argument. It is about whether or not it is \emph{possible} for all the premises to be true and the conclusion false. (Or, to say the same thing in a different way, whether, the conclusion has to be true \textit{if} all of the premises are true.)
