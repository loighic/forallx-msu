...
\begin{ebullet}
\item[] \myanswer{Since \meta{A} is false on every line of a truth table, there is no line on which \meta{A} and \meta{B} are true and \meta{C} is false. Hence, the argument is  valid. (This would be an odd argument, however, since we know that one of the premises is a contradiction.)}
\end{ebullet}
