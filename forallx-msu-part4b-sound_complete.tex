...
\hrule
\medskip
Equivalently, \define{soundness} is a property of a logic system iff, for any sentence, if the sentence is a theorem, then it is a tautology. 
\end{factboxy}
...
\item $\proves M \eif [((P \eand Q) \eor S) \eif M]$
...
\begin{proof}
\hypo{p1}{\enot(M \eor P)} \pr{}
\open
	\hypo{as}{Q} \as{}
	\open
		\hypo{as2}{M} \as{}
		\have{mvp}{M \eor P} \oi{as2}
		\have{nmvp}{\enot(M \eor P)} \reit{p1}
		\close
	\have{nm}{\enot M} \ni{as2-nmvp}
    	\close
\have{con}{Q \eif \enot M} \ci{as-nm}
\end{proof}
\medskip
\end{minipage}
...
\item $\enot(M \eor (P \eor Q)) \proves \enot{Q}$ is valid.\smallskip
...
\item $(\enot R \eiff R) \eor R \proves R$ is valid.\smallskip

\item $F \eand (K \eand R) \proves F \eiff (K \eiff R)$ is valid.\smallskip
...
\item $L, K \eif \enot L \proves \enot K$ is valid.\smallskip 

\item $\proves M \eif ([(P \eand Q) \eor S] \eif M)$ is a theorem.\smallskip
\begin{proof}
\open
	\hypo{as}{M} \as{}
	\open
		\hypo{as2}{(P \eand Q) \eor S} \as{}
		\have{m}{M} \reit{as}
		\close
	\have{p}{((P \eand Q) \eor S) \eif M} \ci{as2-m}
	\close
\have{con}{M \eif (((P \eand Q) \eor S) \eif M)} \ci{as-p}
\end{proof}
\medskip
