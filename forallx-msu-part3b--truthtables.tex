...
An argument is valid when it is the case that \textit{if the premises are true}, then the conclusion has to be true. In the argument above, the premises are true when `$M$' is true and `$L$' is false---which is line 2. That there are other valuations where both premises are not true doesn't matter. Such lines don't violate our definition of \textit{valid}, and so they get a `\cm'. 
