...
Entailment is easy to check with a truth table. Does `$\enot L \eif (M \eor L)$' and `$\enot L$' entail `$M$'? To find out, we check whether there is any valuation that makes both `$\enot L \eif (M \eor L)$' and `$\enot L$' true whilst making `$M$' false.  
\begin{center}
...
Our next step is this important observation:
	\factoidbox{
...
\end{tabular}
\end{center}
The truth values for the premises are the same, and the truth values for the conclusion have, on each line, flipped from T to F or vice versa. Now, when we evaluate each line, what do we find? As before, on lines 1, 3, and 4, one of the premises is false, and so they are not bad lines. On line 2, the premises are true and the conclusion is false. Bad line! That means that $\enot L \eif (M \eor L)$ and $\enot L$ do not entail $\enot M$ and the argument `$\enot L \eif (M \eor L)$, $\enot L \therefore \enot M$' is not valid.  
...
The truth values for each atomic sentence and the connectives are filled in normally. But, as shown in figure \ref{fig:tt-7b}, that will leave one column under the `$\proves$'. That column will also be filled in with `T's and `F's, but there `T' and `F' don't mean `true' and `false'. `T' means that the line is a good line. `F' means that the line is a bad line. (See \S\ref{s:tt-entailment} and \S\ref{s:tt-validity} for the definition of \textit{valid} and an explanation of when lines are good and when they are bad.) If all of the lines are good, then the argument is valid. If one or more of the lines is bad, the argument is invalid. 

In this example, all of the lines are good, and so we have all `T's below the `$\proves$' (figure \ref{fig:tt-9}). Recall from \S\ref{s:tt-validity}, each line is a good line and the argument is valid even though, on the third line, the premise is false and the conclusion is true, and on the second and fourth lines, the premise is false and the conclusion is false. The only scenario that makes a line a bad line is when the premise or premises are true and the conclusion is  false. 
...
Consider the argument in figure \ref{fig:tt-10}: $P \eor R \proves R \eif \enot P$. In the first line of this truth table, the premise is true and the conclusion is false. That's a bad line, and so in the column under the `$\proves$', we put an `F'. Even though all of the other lines are fine, because we have one bad line, the argument is not valid. 
