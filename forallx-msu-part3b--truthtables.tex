...
The truth values for each atomic sentence and the connectives are filled in normally. But, as shown in figure \ref{fig:tt-7b}, that will leave one column under the `$\proves$'. That column will be filled in with these symbols: \checkmark and \text{\sffamily x}. As you might guess, `\checkmark' means that the line is a good line, and `\text{\sffamily x}' means that it is a bad line. (See \S\ref{s:tt-entailment} and \S\ref{s:tt-validity} for the definition of \textit{valid} and an explanation of when lines are good or bad.) If all of the lines are good, then the argument is valid. If one or more of the lines is bad, the argument is invalid. 

In this example, all of the lines are good, and so we have all \checkmark s below the `$\proves$' (figure \ref{fig:tt-9}). Recall from \S\ref{s:tt-validity}, each line is a good line and the argument is valid even though, on the third line, the premise is false and the conclusion is true, and on the second and fourth lines, the premise is false and the conclusion is false. The only scenario that makes a line a bad line is when the premise or premises are true and the conclusion is  false. 
