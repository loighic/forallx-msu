...
\section{Validity}\label{s:tt-validity}\label{s:tt-entailment}
...
Now, we want to know whether this argument satisfies or violates the definition of \textit{valid}. So, to begin, ask yourself, ``When both premises are true, is the conclusion true?'' Or ``Is there any scenario (that is, any valuation) where both premises are true and the conclusion is false?'' If the answer to the first question is always ``yes,'' then the argument is valid. If the answer to the second question is ever ``no,'' then the argument is invalid.

As you can see, there is only one row where both `$\enot L \eif (M \eor L)$' and `$\enot L$' are true, and so that is the only row that concerns us. On that row, `$M$' is also true. Hence, `$\enot L \eif (M \eor L), \enot L \proves M$' is valid. 
\begin{center}
\begin{tabular}{c c|d e e e e f|d f| c | c}
$M$ & $L$ & \enot&$L$&\eif&$(M$&\eor&$L)$& \enot&$L$& \proves & $M$\\
\hline
...
Also (and \textbf{importantly!}), when there is a line where all of the premises are not true, we put a `\cm' beneath the turnstile (whether the conclusion is true or false). Again, recall that an argument is valid when it is the case that \textit{\textbf{if the premises are true}}, then the conclusion has to be true. In the argument above, the premises are true when `$M$' is true and `$L$' is false. That there are other valuations where both premises are not true doesn't matter. Such lines don't violate our definition of \textit{valid}, and so they get a `\cm'. 
...
Now, let's make one small (but significant) change to the argument: $\enot L \eif (M \eor L)$, $\enot L \proves \enot M$. The premises are the same, but now the conclusion is $\enot M$ instead of $M$. Here is the truth table:
\begin{center}
\begin{tabular}{c c|d e e e e f|d f| c | c}
...
\end{tabular}
\end{center}
This means that `$\enot L \eif (M \eor L)$, $\enot L \proves \enot M$' is not valid.

\begin{factboxy2}{good and bad lines}
Let's call lines that violate the definition of \define{valid} \textit{bad lines}.
	\begin{enumerate}
		...
		\item[(4)] Any line where at least one premise is false cannot be a bad line. (So, whatever the case may be with the other premises and the conclusion, \textbf{it's a good line}.) Put a `\cm' on that line.
	\end{enumerate}
\end{factboxy2}
