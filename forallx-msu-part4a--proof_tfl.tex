...
\end{proof}
\end{factboxy}  
\noindent To use this rule, we need the \textit{negation} of one of the disjuncts. The negation of a sentence in TFL is the sentences with a $\enot$ in front of it. The negation of `$\meta{B}$' is `$\enot\meta{B}$', and the negation of `$\enot\meta{B}$' is `$\enot\enot\meta{B}$' (and so on by the same pattern). Hence, to use the disjunction elimination rule right now, we need to transform `$P$' into `$\enot\enot P$' with the double negation rule.
