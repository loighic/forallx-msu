...
\end{proof}
\tcblower
\footnotesize{The `$m$' and `$n$' will never appear in an actual proof. In a proof, the lines are numbered $1$, $2$, $3$, etc. 
The `$m$' and `$n$' are used here to indicate that $\meta{A}$ and $\meta{B}$ can be on any lines in the proof.}
\end{factboxy}
