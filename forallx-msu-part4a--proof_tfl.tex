...
The \define{double negation rule} is a rule of convenience that sometimes compliments the disjunction-elimination rule. (There are also times when it will be used in the proofs that are discussed in \ref{s:theorems}, but, for the material in this chapter, it will only be used with the disjunction-elimination rule.) First, notice that the disjunction-elimination rule is very specific. To use this rule, we need, on one line of our proof, a sentence of the form $\meta{A}\eor\meta{B}$, and on another line of our proof we need one side of the disjunction---$\meta{A}$ or $\meta{B}$---with a `$\enot$' in front of it: $\enot\meta{A}$ or $\enot\meta{B}$. 

This presents a problem if we these two sentences somewhere in a proof:
\begin{proof}
	\hypo{ab}{\enot P \eor Q} \pr{}
	\hypo{nb}{P} \pr{}
\end{proof}
You might, then, think that we can put `$Q$' on a new line.
\begin{proof}
	\hypo{ab}{\enot P \eor Q} \pr{}
	\hypo{nb}{P} \pr{}
	\have{q}{Q} \oe{ab,nb}
\end{proof}
In a sense, this is the right idea for the disjunction-elimination rule---one side of the disjunction `$\enot P \eor Q$' has to be true and the `$P$' on line 4 means that `$\enot P$' is false. Hence, we should be allowed to put `$Q$' on a line by itself. The disjunction-elimination rule, however, do not permit this. As we just said, the rule requires that we have, on a line somewhere, one side of the disjunction with a `$\enot$' before it. The double negation rule helps us with this so that we can use the disjunction-elimination rule more often. 

Before see how the double negation rule can help us with our derivation, let's see the rule. The first version of the double negation rule allows us to add two \textit{not}s (i.e., \textit{not not}) to an atomic sentence---which, of course, will not change the sentence's truth value. The second version of the double negation rule allows us to remove two \textit{not}s, although needing to do this is less common.
...
With this rule, we can derive `$Q$' this way:
...
\begin{proof}
	...
	\have{q}{Q} \oe{ab,nnb}
\end{proof}
The `$P$' on line 2 and the `$\enot \enot P$' have exactly the same meaning. The only difference between `$P$' and `$\enot \enot P$' is their form. But now that we have `$\enot \enot P$', we have what is on the left side of the disjunction with a `$\enot$' before it.
\begin{proof}
	\hypo{ab}{\textbf{\textit{\textcolor{red2}{\enot P}}} \eor Q} \pr{}
	\hypo{nb}{P} \pr{} 
	\have{nnb}{ \textbf{\textit{\enot \textcolor{red2}{\enot P}}} } \dn{nb}
	\have{q}{Q} \oe{ab,nb}
\end{proof}
...
\section{Some examples with negative introduction and elimination}

\textit{These two are from my week 12 notes. Put a couple from week 11 before them and one from week 12 with a sub-proof within subproof after them.}
