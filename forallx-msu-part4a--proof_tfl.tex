...
Armed with our `$\proves$' symbol, we can introduce some more terminology. To say that there is a proof of $\meta{A}$ with no undischarged assumptions---that is, in particular, with no premises---we write: ${} \proves \meta{A}$. In this case, we say that $\meta{A}$ is a \define{theorem}.
